...
for i = 1:length(image_names)
    ...
    %choose SURF feature points
    points = detectSURFFeatures(img(:,:,1));
    size(points);
    strongPoints = points.selectStrongest(alpha);
    pixelCoord = round(strongPoints.Location);
    
    if not(isempty(pixelCoord))
        index(1:length(pixelCoord)) = (pixelCoord(:,2)-1).*size(img,2) + pixelCoord(:,1);
    end
    
    size(filter_response(index,:));
    
    %choose pixel fiter response
